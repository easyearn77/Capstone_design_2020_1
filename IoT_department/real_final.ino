...
#include <LedControl.h> // led 도트 매트릭스 사용을 위한 라이브러리 선언 
#include <stdlib.h>
#include "LedControl.h" // 라이브러리 사용 선언(도트 매트릭스 사용)
#define DEBUG true
...
void setup() {
  ...
  sendData("AT+CWJAP=\"espressovivian\",\"shoumunzmay\"\r\n", 5000, DEBUG); //사용할 공유기 설정
}
 
void loop() {
  float duration, distance;
  String latitude="37.4679917";
  String longitude="127.0434159";
  ...
  //안전바 사이의 거리(100cm)보다 작게 되면 서버에 위치를 전송합니다.
  if(distance<100){
    ...
    cmd += "192.168.0.1"; // 서버 IP(local-host사용)
    cmd += "\",8080";           // 포트: 8080
    esp8266.println(cmd);
...
else
{
 ...
 delay(1000);//1초마다 out of sight 출력
